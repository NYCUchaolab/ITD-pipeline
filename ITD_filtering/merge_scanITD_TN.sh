...
Normal_sample_file=${1}  # [1] Normal Sample ID
Tumor_sample_file=${2}   # [2] Tumor Sample ID
case_ID=${3}             # [3] Sample Case ID

output_file=./scanITD/${case_ID}.itd.filter.de.tsv
...
do
  normal_chr=${normal_line[0]}    # Chromosome (index 0)
  normal_start=${normal_line[1]}  # Start position (index 1)
  normal_end=$(echo "${normal_line[7]}" | grep -oP 'END=\K[0-9]+' || echo 0)
...
do
  tumor_chr=${tumor_line[0]}    # Chromosome (index 0)
  tumor_start=${tumor_line[1]}  # Start position (index 1)
  tumor_end=$(echo "${tumor_line[7]}" | grep -oP 'END=\K[0-9]+' || echo 0)
  ...
  do
    ...
    # Check if the chromosome matches before proceeding with overlap check
...
echo -e "${file_ID}\t${sample_ID} merge TN down" >> "tmp/scanITD.out"