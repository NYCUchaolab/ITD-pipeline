...
# : <<'#__COMMENT_OUT__'
  # 
  echo "${PATH_TO_SAMTOOLS}/samtools view -@ $threadN -h -F $SAM_FILTERING_FLAG -q $THRES_MAP_QUALITY $INPUTBAM > ${TRDIR}/tmp/temp.input.sam"
  ${PATH_TO_SAMTOOLS}/samtools view -@ $threadN -h -F $SAM_FILTERING_FLAG -q $THRES_MAP_QUALITY $INPUTBAM > ${TRDIR}/tmp/temp.input.sam
  check_error $?
  #
  ...
  check_error $?
  #
  echo "${PATH_TO_BLAT}/pblat -threads=$threadN -stepSize=5 -repMatch=2253 -minScore=20 -ooc=${PATH_TO_BLAT_OOC} $PATH_TO_BLAT_REF ${TRDIR}/tmp/candJunc.fa ${TRDIR}/tmp/candJunc.psl"
  ${PATH_TO_BLAT}/pblat -threads=$threadN -stepSize=5 -repMatch=2253 -minScore=20 -ooc=${PATH_TO_BLAT_OOC} $PATH_TO_BLAT_REF ${TRDIR}/tmp/candJunc.fa ${TRDIR}/tmp/candJunc.psl
  check_error $?
  #
...
# : <<'#__COMMENT_OUT__'
  ...
  if [ -s ${TRDIR}/tmp/itdContig.fa ]; then
    # echo "${PATH_TO_BLAT}/pblat -stepSize=5 -repMatch=2253 -maxIntron=${THRES_MAX_ITD_LENGTH} ${PATH_TO_BLAT_REF} ${TRDIR}/tmp/itdContig.fa ${TRDIR}/tmp/itdContig.psl"
    # ${PATH_TO_BLAT}/pblat -threads=16 -stepSize=5 -repMatch=2253 -maxIntron=${THRES_MAX_ITD_LENGTH} ${PATH_TO_BLAT_REF} ${TRDIR}/tmp/itdContig.fa ${TRDIR}/tmp/itdContig.psl
    #
    echo "${PATH_TO_BLAT}/pblat --threads=$threadN -stepSize=5 -repMatch=2253 -maxIntron=1000 ${PATH_TO_BLAT_REF} ${TRDIR}/tmp/itdContig.fa ${TRDIR}/tmp/itdContig.psl"
    ${PATH_TO_BLAT}/pblat -threads=$threadN -stepSize=5 -repMatch=2253 -maxIntron=1000 ${PATH_TO_BLAT_REF} ${TRDIR}/tmp/itdContig.fa ${TRDIR}/tmp/itdContig.psl
    check_error $?
  else 
...
if [ -s ${TRDIR}/tmp/juncListitd14.txt ]; then
  ...
  check_error $?
  #
  echo "${PATH_TO_SAMTOOLS}/samtools view -@ $threadN -h -b -F $SAM_FILTERING_FLAG -q $THRES_MAP_QUALITY -L ${TRDIR}/tmp/temp.range.bed $INPUTBAM > ${TRDIR}/tmp/temp.range.bam"
  ${PATH_TO_SAMTOOLS}/samtools view -@ $threadN -h -b -F $SAM_FILTERING_FLAG -q $THRES_MAP_QUALITY -L ${TRDIR}/tmp/temp.range.bed $INPUTBAM > ${TRDIR}/tmp/temp.range.bam
  check_error $?
  #
  echo "${PATH_TO_SAMTOOLS}/samtools index -@ $threadN ${TRDIR}/tmp/temp.range.bam"
  ${PATH_TO_SAMTOOLS}/samtools index -@ $threadN ${TRDIR}/tmp/temp.range.bam
  check_error $?
